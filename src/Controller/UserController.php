...
class UserController
{
    ...
    public function put(Request $request, Response $response, array $args): Response
    {
        ...
        // realname
        if (isset($req_data['realname'])) {
            $usuarioId = $this->findIdBy('realname', $req_data['realname']);
            if ($usuarioId && intval($args['userId']) !== $usuarioId) {
                // 400 BAD_REQUEST: username already exists
                return Error::error($response, StatusCode::STATUS_BAD_REQUEST);
            }
            $user->setRealname($req_data['realname']);
        }

        // birthDate
        if (isset($req_data['birthDate'])) {
            $usuarioId = $this->findIdBy('birthDate', $req_data['birthDate']);
            if ($usuarioId && intval($args['userId']) !== $usuarioId) {
                // 400 BAD_REQUEST: username already exists
                return Error::error($response, StatusCode::STATUS_BAD_REQUEST);
            }
        ...
        // isActive
        if (isset($req_data['isActive'])) {
            $usuarioId = $this->findIdBy('isActive', $req_data['isActive']);
            if ($usuarioId && intval($args['userId']) !== $usuarioId) {
                // 400 BAD_REQUEST: username already exists
                return Error::error($response, StatusCode::STATUS_BAD_REQUEST);
            }
